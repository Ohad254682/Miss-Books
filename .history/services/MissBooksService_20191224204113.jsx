...
export default { getBooks, getBookById, addReview, deleteReview, getListOfGoogleBooks, addBook }

gBooks = StorageService.load('gBooks');
...
function addReview(id, name, rate, date, comment) {
    let editBook = gBooks.find(book => book.id === id)
    editBook = { ...editBook };
    var newReview = { name: name, rate: rate, date: date, comment: comment };
    if (!editBook.reviews) editBook.reviews = [];
    editBook.reviews = [...editBook.reviews, newReview]
    gBooks = gBooks.map(book => editBook.id === book.id ? editBook : book);
    StorageService.store('gBooks', gBooks);
...
function getListOfGoogleBooks(searchTerm) {
    var resultsGoogleBooks = [];
    return getGoogleBooks()
        .then(googleBooks => {
            resultsGoogleBooks = googleBooks.data.items.filter(googleBook => googleBook.volumeInfo.title.includes(searchTerm))
            console.log(resultsGoogleBooks);
            return resultsGoogleBooks;
        });
}

function addBook(book) {
    gBooks = [...gBooks, addNewBook(book)];
    StorageService.store('gBooks', gBooks);
    return Promise.resolve(true);
}
